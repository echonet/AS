...
parser.add_argument("--data_dir", type=str, required = True, help= "Path to the Dicom file dir. data_dir/{studyid}/AV_doppeler/{dicomfiles}")
parser.add_argument("--output_path_folders", type=str, default='./predictions', help= "Output dir. Default is './predictions'")
...
weights_path = f"./weights/{model}_weights.ckpt"
...
data_dirs = list(data_dir.glob("*/AV_doppler"))
DICOM_FILES = [f for d in data_dirs for f in d.iterdir()]
print(f"number of DICOMs {model}:\t{len(DICOM_FILES)}")
...
for DICOM_FILE in DICOM_FILES:
    try:
        ...
        results.append({
            ...
            "studyid": Path(DICOM_FILE).parents[1].name,
            "measurement_name": 'avvmax',
...
if args.output_path_folders:
    metadata.to_csv(os.path.join(OUTPUT_FOLDERS, f"metadata_{model}.csv"), index=False)
